...
export function* updatePatrimonioRequest(action) {
  ...
  try {
    ...
    if (data.tipo === 'passivo') {
      data.parcelas = moment(data.dataFinal).diff(moment().utc(), 'months') + 1;
      data.total =
        (moment(data.dataFinal).diff(moment().utc(), 'months') + 1) * data.pmt;

      body.parcelas = moment(body.dataFinal).diff(moment().utc(), 'months') + 1;
      body.total =
        (moment(body.dataFinal).diff(moment().utc(), 'months') + 1) * body.pmt;
      ...
      local.patrimonios.passivos.list = local.patrimonios.passivos.list.map(
        p => {
...
export function* addPatrimonioRequest(action) {
  try {
    ...
    local.patrimonios.patrimonioLiquido +=
      data.tipo === 'passivo' ? -1 * data.total : data.valor;
    localStorage.setItem('@Ondazul: data', JSON.stringify(local));
